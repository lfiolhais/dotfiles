...
vim.pack.add({
    ...
    { src = "https://github.com/linrongbin16/gitlinker.nvim",             name = "gitlinker"},
...
-- Get Contact
vim.keymap.set('n', '<leader>gc', '<cmd>GetContact<cr>', key_opts)
...
vim.api.nvim_create_user_command('GetContact',
    function (local_opts, opts)
      local_opts = vim.tbl_deep_extend('force', { tool = nil }, local_opts or {})
      local cmd = {'khard', 'email', '-a', 'work', '--parsable', '--remove-first-line'}
      ...
      local default_opts = {
          source = {
              ...
              choose = function(item)
                  vim.api.nvim_buf_set_lines(buf, row, row, false, { item })
              end,
          }
      }
      ...
      return MiniPick.builtin.cli({
          ...
          postprocess = function(items)
            ...
            for key,value in pairs(items) do
                ...
                results[key] = split[2] .. " <" .. split[1] .. ">"
            end
            return results
          end
      }, opts)
    end,
    {desc = "Select contact from khard."}
)
